...
const loadRestaurantNetworkFirst = id => {
  ...
  DBHelper.getServerData(endpointRestaurantById).then(dataFromNetwork => {
    self.restaurant = dataFromNetwork;
    updateRestaurantUI();
    createBreadcrumb();
    saveRestaurantsDataLocally(dataFromNetwork).then(() => {
    ...
    getLocalRestaurantByIdData(id).then(offlineData => {
...
const createResponsivePicture = restaurant => {
  ...
  const srcsetJPEG = `${DBHelper.getImageUrlForRestaurant(restaurant, 'jpeg', 300)} 300w,
